...
class Status(enum.Enum):
    ...
    UNLOAD_ONLY = 1  # must unload only
...
class CartCtl:
    ...
    def try_load_here_single(self):
        ...
        for request_idx in range(len(self.requests)):
            ...
            if request.src == self.cart.pos and request.weight <= free_cap:
                if self.status != Status.UNLOAD_ONLY:
    ...
    def find_load_there_single(self, priority=False):
        """
        Finds a single slot and requests that fits in to the cart.
    ...
    def find_prio_request(self):
        "return prioritized cargo waiting in requests; or None"
        for cargo_req in self.requests:
            if cargo_req.prio:
                return cargo_req
        return None
    ...
    def heartbeat(self):
        ...
        # nothing load or unload at this station
